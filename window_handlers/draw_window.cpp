...
void drawWindow(AppState& state) {
    // Рисуем фон главного окна приложения
    ClearBackground(LIGHTGRAY);

    int screenW = GetScreenWidth(); // Получаем ширину экрана для адаптивного размещения элементов
    int screenH = GetScreenHeight(); // Получаем высоту экрана для адаптивного размещения элементов
    int rightX = PANEL_W; // Вычисляем координату X для правой части окна, где будут располагаться график и гистограмма
    int rightW = screenW - PANEL_W; // Вычисляем ширину правой части окна, учитывая ширину панели
    int plotH = static_cast<int>(screenH * 0.60f); // Вычисляем высоту области для графика сигнала, занимая 60% от высоты экрана
    int histH = screenH - plotH; // Вычисляем высоту области для гистограммы, занимая оставшиеся 40% от высоты экрана
