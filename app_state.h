...
struct AppState {
    ...
    void syncParamsToInputs(); // Синхронизация параметров сигналов в текстовые поля (для отображения текущих значений параметров пользователю)
    void syncInputsToParams(); // Синхронизация текстовых полей обратно в параметры сигналов (после редактирования пользователем)
